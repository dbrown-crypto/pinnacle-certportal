...
jwkclient = PyJWKClient(JWKS_URL) if JWKS_URL else None
    ...
    try:
              ...
              if alg.startswith(("ES", "RS")):
                            if not jwkclient:
                                          ...
                                          key = jwkclient.get_signing_key_from_jwt(token).key
...
async def issue_certificate(body: IssueRequest, authorization: Optional[str] = Header(None)):
    ...
    if result.route == "route_to_agent":
              ...
              if RESEND_KEY:
                ...
                pass  # special_requests row is source of truth; email is a nudge
    ...
    # Email is the convenience copy; the cert is the product.
    # A delivery failure must NEVER discard a cert the client successfully issued.
